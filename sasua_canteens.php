...
class SASUA_Canteens extends SASUA_Canteens_Object
{
        ...
        public function get( $zone, $type, $format = 'xml', $cached = true )
        {
                ...
                $ckey = SASUA_Canteens_Cache::key( array( $this->zone, $this->type ) );
        ...
        public function fetch( $zone = null, $type = null )
        {
                ...
                $tidy = new tidy();
                $content = $tidy->repairString( $content, array( 'wrap' => 0 ) );
        ...
        private function __parseDate( $date, $regex, $format )
        {
                ...
                if (preg_match( $regex, $date, $matches )) {
                        $date = array_combine( $format, array( $matches[1], $matches[2], $matches[3] ) );
                        if (is_numeric( $date['m'] )) {
                                $date = implode( $separator, array( $date['m'], $date['d'], $date['y'] ) );
                        } else {
                                $date = implode( ' ', array( $date['d'], SASUA_Canteens_Utility::translateMonthPT2EN( $date['m'] ), $date['y'] ) );
                        }
 ...
 * This cache class has no expire mechanism, we append today's date
 * to cache file, making it last until midnight of that day
 *
 * @package SASUA_Canteens
...
class SASUA_Canteens_Menus extends SASUA_Canteens_Menu_Object
{
        ...
        public function asXML( $xmlDecl = false, $encoding = 'utf-8' )
        {
                ...
                return $xmlObj->document( null, array( 'xml_declaration' => $xmlDecl, 'encoding' => $encoding ) );
        }
...
abstract class SASUA_Canteens_Menu_Object
{
        ...
        {
                $sxe = $this->asXMLObj();
                return $sxe->document( null, array( 'xml_declaration' => $xmlDecl, 'encoding' => $encoding ) );
        }
